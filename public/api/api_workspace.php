...
try {
    ...
    if ($method == "GET") {
        ...
        // --- INÍCIO DA SIMULAÇÃO DE MÉTODOS (COMPATIBILIDADE INFINITYFREE) ---
        $simulated_method = isset($data['_method']) ? strtoupper($data['_method']) : null;

        if ($simulated_method === 'PATCH') {
            // Ação: Trocar o workspace ativo.
            $id_workspace = $data['id'] ?? null;
            if (!$id_workspace) throw new Exception("ID do workspace não fornecido para troca.");
        ...
        if ($simulated_method === 'PUT') {
            // Ação: Salvar o layout do workspace (autosave).
            $id_workspace = $data['id'] ?? null;
            $layout_data = $data['layout_data'] ?? null;
            if (!$id_workspace || !$layout_data) throw new Exception("Dados insuficientes para atualizar.");
        ...
        if ($simulated_method === 'DELETE') {
            // Ação: Excluir um workspace.
            $id_workspace = $data['id'] ?? null;
            if (!$id_workspace) throw new Exception("ID do workspace não fornecido para exclusão.");
        ...
        // Se não for uma simulação, continua com a lógica normal de POST (criar/renomear)
