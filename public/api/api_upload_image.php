...
$uploadDir = '../uploads/'; // CORREÇÃO: A pasta uploads está um nível acima da pasta /api/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($file['tmp_name'], $uploadFilePath)) {
        ...
        // CORREÇÃO: Constrói a URL pública correta
        // O caminho base agora é /foco-total/public/
        $basePath = '/foco-total/public/';
        $fileUrl = $protocol . '://' . $host . $basePath . 'uploads/' . $uniqueName;
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    // CORREÇÃO: Remove o caminho base da URL para obter o caminho do arquivo no servidor
    $basePath = '/foco-total/public/';
    if (strpos($filePath, $basePath) === 0) {
        // O caminho do arquivo no servidor será 'uploads/nome_do_arquivo.jpg'
        $serverFilePath = '../' . substr($filePath, strlen($basePath));
    }

    // VALIDAÇÃO DE SEGURANÇA CRÍTICA:
    // Garante que o arquivo a ser deletado está DENTRO da pasta 'uploads'
    $realUploadDir = realpath($uploadDir);
    $realFilePath = isset($serverFilePath) ? realpath($serverFilePath) : false;
