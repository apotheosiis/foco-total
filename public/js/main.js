document.addEventListener('DOMContentLoaded', function () {
    ...
    let currentEditingPomodoroId = null; // NOVO: Guarda o ID do pomodoro que está sendo editado
    let globalSettings = {};
    ...
    function saveGlobalSettings() {
        // CORREÇÃO: Esta função agora salva as configurações de um pomodoro específico.
        if (!currentEditingPomodoroId) return;

        const pomodoroState = widgetState.get(currentEditingPomodoroId);
        if (!pomodoroState) return;

        // Atualiza o estado do widget específico
        pomodoroState.settings.foco = parseInt(focoDurationInput.value, 10) || 25;
        pomodoroState.settings.shortBreak = parseInt(shortBreakDurationInput.value, 10) || 5;
        pomodoroState.settings.longBreak = parseInt(longBreakDurationInput.value, 10) || 30;
        pomodoroState.settings.longBreakInterval = parseInt(longBreakIntervalInput.value, 10) || 4;
        
        // O volume ainda pode ser global, pois é uma configuração do app, não do timer.
        globalSettings.volume = parseFloat(volumeSlider.value);
        localStorage.setItem('focoTotalSettings', JSON.stringify({ volume: globalSettings.volume })); // Salva apenas o volume globalmente

        // Dispara um evento para notificar o widget específico que suas configurações mudaram
        window.dispatchEvent(new CustomEvent('pomodoroSettingsUpdated', { detail: { widgetId: currentEditingPomodoroId } }));

        saveDashboard(); // Salva o estado do dashboard com as novas configurações do widget
    ...
    function loadGlobalSettings() {
        ...
        // Apenas o volume é realmente global agora
        globalSettings = {
    ...
    // ======================= LÓGICA DE ÁUDIO =======================
    const sounds = {
        focoEnd: new Audio('/foco-total/public/audio/foco_end.mp3'),
        breakEnd: new Audio('/foco-total/public/audio/break_end.mp3'),
        longBreakEnd: new Audio('/foco-total/public/audio/long_break_end.mp3')
    };
    ...
    function initializeWidget(contentEl, type, widgetData, wrapperEl) {
            ...
            deleteBtn.addEventListener('click', async () => {
                ...
                if (type === 'photo') {
                    ...
                    if (bgImage && bgImage.startsWith('url("')) {
                        ...
                        try {
                            await fetch('/foco-total/public/api/api_upload_image.php', {
                                method: 'DELETE',
        ...
        if (type === 'pomodoro') { initPomodoroWidget(contentEl, widgetData); } 
    ...
    function initPomodoroWidget(widgetEl, widgetData) {
        ...
        // CORREÇÃO FINAL: Usa as configurações salvas em `widgetData` ou cria um novo estado padrão.
        const widgetId = widgetEl.closest('.grid-stack-item').getAttribute('gs-id');
        const state = {
            mode: currentMode,
            count: pomodoroCount,
            // Se `widgetData.settings` existe (veio do banco), usa ele. Senão, cria um novo objeto padrão.
        ...
        function switchMode(newMode) {
            ...
            if (newMode === 'work') timeLeft = state.settings.foco * 60;
            if (newMode === 'short-break') timeLeft = state.settings.shortBreak * 60;
            if (newMode === 'long-break') timeLeft = state.settings.longBreak * 60;
            ...
            timerInterval = setInterval(() => {
                ...
                if (timeLeft < 0) {
                    ...
                    if (currentMode === 'work') {
                        ...
                        pomodoroCount++;
                        if (pomodoroCount > 0 && pomodoroCount % state.settings.longBreakInterval === 0) {
        ...
        // CORREÇÃO: Abre o modal e o prepara para este widget específico.
        if(pomodoroSettingsBtn) {
            pomodoroSettingsBtn.addEventListener('click', () => {
                currentEditingPomodoroId = widgetId; // Informa qual widget está sendo editado
                // Preenche o modal com as configurações deste widget
                focoDurationInput.value = state.settings.foco;
        ...
        // Ouve o evento de atualização e verifica se é para este widget
        window.addEventListener('pomodoroSettingsUpdated', (e) => {
            if (e.detail.widgetId === widgetId) switchMode(currentMode);
        });
    ...
    function initPhotoWidget(widgetEl) {
        ...
        photoInput.addEventListener('change', async (event) => {
            ...
            try {
                ...
                const response = await fetch('/foco-total/public/api/api_upload_image.php', {
                    method: 'POST',
                    body: formData
                });
    ...
    function initTextBlockWidget(contentEl, widgetData, wrapperEl) {
        ...
        // --- INÍCIO DA CORREÇÃO ---
        // Usamos "debounce" para salvar apenas quando o usuário para de digitar.
        let debounceTimer;
        ...
        quill.on('text-change', () => {
            ...
            // Configura um novo timer para salvar após 1.5 segundos de inatividade
    ...
    function getDashboardData() {
        ...
        grid.engine.nodes.forEach(node => {
            ...
            if (state !== undefined) {
                ...
                if (type === 'pomodoro') {
                    data.settings = state.settings; // Salva as configurações do pomodoro
                }
    ...
    async function saveDashboard() {
        ...
        if (activeWorkspaceId) {
            try {
                // CORREÇÃO: Usar POST para simular o PUT, garantindo compatibilidade com InfinityFree.
                await fetch('/foco-total/public/api/api_workspace.php', {
                    method: 'POST', // MUDADO DE 'PUT' PARA 'POST'
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        _method: 'PUT', // CAMPO DE SIMULAÇÃO
                        id: activeWorkspaceId, layout_data: JSON.stringify(widgetsData) 
                    })
                });
            } catch (error) {
    ...
    async function loadDashboard() {
        try {
            const response = await fetch('/foco-total/public/api/api_workspace.php');
            ...
            if (workspaces.length > 0) {
                ...
                try {
                    const newWorkspaceResponse = await fetch('/foco-total/public/api/api_workspace.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ 
    ...
    async function loadWorkspace(workspaceId, isInitialLoad = false) {
        try {
            ...
            if (!isInitialLoad) {
                // CORREÇÃO: Usar POST para simular o PATCH, garantindo compatibilidade com InfinityFree.
                await fetch('/foco-total/public/api/api_workspace.php', {
                    method: 'POST', // MUDADO DE 'PATCH' PARA 'POST'
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        _method: 'PATCH', // CAMPO DE SIMULAÇÃO
                        id: workspaceId 
                    })
                });
            ...
            const response = await fetch('/foco-total/public/api/api_workspace.php?id=' + workspaceId);
            if (!response.ok) {
        ...
        workspaces.forEach(ws => {
            ...
            li.querySelector('.delete-ws-btn').addEventListener('click', async (e) => {
                ...
                if (confirm(`Tem certeza que deseja excluir o workspace "${ws.nome}"? Esta ação não pode ser desfeita.`)) {
                    try {
                        // CORREÇÃO: Usar POST para simular o DELETE.
                        const response = await fetch('/foco-total/public/api/api_workspace.php', {
                            method: 'POST', // MUDADO DE 'DELETE' PARA 'POST'
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ 
                                _method: 'DELETE', // CAMPO DE SIMULAÇÃO
                                id: ws.id })
                        });
                        if (response.ok) {
                            li.remove();
